    ...
    <div>
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column label="温度">
                <template #default="scope">
                    <el-link type="primary">{{ currentTemp }}℃</el-link>
                </template>
            </el-table-column>
            <el-table-column label="所属学院">
                <template #default="scope">
                    <el-tag :color="CollegesColorType[scope.row.college_type]">{{college_method( scope.row.college_type )}}</el-tag>
                </template>
            </el-table-column>
        ...
        <el-dialog v-model="dialogVisible" title="实验室位置"
        :fullscreen="true" v-if="dialogVisible">
        ...
        <!-- 更新 -->
         <el-dialog v-model="dialogUpdateVisible" title="更新实验室">
            ...
            <el-form class="updateForm" ref="updateFormRef" style="max-width: 
        600px" :model="updateForm" :rules="rules" status-icon>
                ...
                <el-form-item label="实验室类型" prop="lab_type">
                    <el-select v-model="updateForm.lab_type" placeholder="请选择实验室类型" style="width: 240px">
                ...
                <el-form-item label="所属学院" prop="college_type">
                    <el-select v-model="updateForm.college_type" placeholder="请选择学院" style="width: 240px">
                        <el-option v-for="item in CollegeTpye" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                </el-form-item>
            </el-form>
...
import { CollegeTpye,LabType,CollegesColorType,LabColorType } from '../../util/type';
...
onMounted(()=>{
    getList()
     setInterval(async () => {
        try {
            const res = await axios.get('http://localhost:8088/api/temp');
            currentTemp.value = res.data;
        } catch (err) {
            console.log("请求温度失败：", err);
        }
    }, 1000);
})

const college_method=(value)=>{
    var obj=CollegeTpye.find(item=>item.value===value)
    return obj.label
}
